...
vc_grids_presentation <- function(data, gam_list, binomial_gam){
  nlat = 80
  nlon = 120
  ...
  spatial_grid$pred1 <- exp(predict(gam_list[[2]], newdata = spatial_grid, type = "response"))
  spatial_grid$se1 <- exp(predict(gam_list[[2]], newdata = spatial_grid, se = T, type = "response")[[2]])
  spatial_grid$pred2 <- exp(predict(binomial_gam, newdata = spatial_grid, type = "response"))
  spatial_grid$se2 <- exp(predict(binomial_gam, newdata = spatial_grid, se = T, type = "response")[[2]])
  ...
  spatial_grid$pred3 <- exp(predict(gam_list[[2]], newdata = spatial_grid, type = "response"))
  spatial_grid$se3 <- exp(predict(gam_list[[2]], newdata = spatial_grid, se = T, type = "response")[[2]])
  spatial_grid$pred4 <- exp(predict(binomial_gam, newdata = spatial_grid, type = "response"))
  spatial_grid$se4 <- exp(predict(binomial_gam, newdata = spatial_grid, se = T, type = "response")[[2]])
  ...
  phenology_grid$pred1 <- predict(gam_list[[2]], newdata = phenology_grid, type = "response")
  phenology_grid$se1 <- predict(gam_list[[2]], newdata = phenology_grid, se = T, type = "response")[[2]]
  phenology_grid$pred2 <- predict(binomial_gam, newdata = phenology_grid, type = "response")
  phenology_grid$se2 <- predict(binomial_gam, newdata = phenology_grid, se = T, type = "response")[[2]]
  ...
  phenology_grid$pred3 <- predict(gam_list[[2]], newdata = phenology_grid, type = "response")
  phenology_grid$se3 <- predict(gam_list[[2]], newdata = phenology_grid, se = T, type = "response")[[2]]
  phenology_grid$pred4 <- predict(binomial_gam, newdata = phenology_grid, type = "response")
  phenology_grid$se4 <- predict(binomial_gam, newdata = phenology_grid, se = T, type = "response")[[2]]

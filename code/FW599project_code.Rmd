...
```{r}
brt_pkegg9 <- gbm.step(data = pkegg_train_data,
                     gbm.x = c(7, 11:12, 19),
                     gbm.y = 21,
                     family = 'gaussian',
                     tree.complexity = 10,
                     learning.rate = 0.005,
                     bag.fraction = 0.25)
length(brt_pkegg9$fitted)
summary(brt_pkegg9)
```

Attempt dropping variables
```{r}
pkegg_simp <- gbm.simplify(brt_pkegg9, n.drops = 5)
summary(pkegg_simp)
```

Clear that no variables should be dropped
```{r}
pkegg_final <- brt_pkegg9
```

...
AIC(pkegg_gams_t[[1]][[1]]) - AIC(pkegg_gams_t[[1]][[3]]) # 222   
summary(pkegg_gams_t[[1]][[1]])
summary(pkegg_gams_t[[1]][[2]])
pkegg_gams_t <- replace(pkegg_gams_t, which)

# Remove salinity due to insignificance and model improvement
better_pkegg <- gam(data = pk_egg, 
                    formula = catch ~ s(year, bs = "re") + 
                      s(doy, k = 9) + 
                      te(lon, lat) + s(roms_temperature, k = 9) + 
...
pkegg_gams_t_final <- list(pkegg_gams_t[[1]], better_pkegg)
```
...
par(mfrow = c(2, 2))
plot(pkegg_gams_t_final[[1]][[2]], select = 2, main = "DOY")
plot(pkegg_gams_t_final[[1]][[2]], select = 4, main = "Temperature")
plot(pkegg_gams_t_final[[1]][[2]], select = 5, main = "Salinity")
plot(pkegg_gams_t_final[[1]][[2]], select = 6, main = "DOY by Mean ROMS SST")
dev.copy(jpeg, here('results/pollock_hindcast', 'pollock_egg_roms_monthvc_t.jpg'), 
         height = 10, width = 10, units = 'in', res = 200)
dev.off()

par(mfrow = c(2, 2))
gam.check(pkegg_gams_t_final[[1]][[2]])
...
```{r}
par(mfrow = c(2, 2))
plot(pkegg_gams_t_final[[1]][[3]], select = 2, main = "DOY")
plot(pkegg_gams_t_final[[1]][[3]], select = 4, main = "Temperature")
plot(pkegg_gams_t_final[[1]][[3]], select = 6, main = "Location by Mean ROMS SST")
dev.copy(jpeg, here('results/pollock_hindcast', 'pollock_egg_roms_spacevc_t.jpg'), 
         height = 10, width = 10, units = 'in', res = 200)
dev.off()

par(mfrow = c(2, 2))
gam.check(pkegg_gams_t_final[[1]][[3]])
...
var_ratio_pkegg_month_t <- (summary(pk_egg_base_t)$scale - 
                         summary(pkegg_gams_t_final[[1]][[2]])$scale) / summary(pk_egg_base_t)$scale
var_ratio_pkegg_month_t #0.110

var_ratio_pkegg_space_t <- (summary(pk_egg_base_t)$scale - 
                         summary(pkegg_gams_t_final[[1]][[3]])$scale) / summary(pk_egg_base_t)$scale
var_ratio_pkegg_space_t #0.130
```
...
```{r}
pkegg_vcgrids_t <- vc_grids_presentation(pk_egg, pkegg_gams_t_final)
```
...
par(mfrow = c(1, 3), 
    mai = c(0.7, 0.6, 0.4, 0.4),
    family = "serif")
map_vc_presentation(pk_egg, pkegg_gams_t_final[[2]], pkegg_vcgrids_t)
legend(50,
       5,
...
par(mfrow = c(1, 2),
    ...
    oma = c(3, 1, 1, 1),
    mgp = c(9, 4, 0))
plot_variable(pkegg_gams_t_final,
              covariate = 2,
              bounds = c(-6.5, 9),
              "Day of Year",
...
```{r, warning = FALSE}
pred_pkegg_t <- variable_coefficient2(pkegg_gams_t_final, pk_egg)

jet.colors <- colorRampPalette(alpha = TRUE,
                               c(sequential_hcl(alpha = 0.8, 15, palette = "Mint")))

plot_var_coef(pkegg_gams_t_final, pk_egg, pred_pkegg_t)
...
ratio_pklarvae_phenology_t # 0.078
...
ratio_pklarvae_geography_t # 0.163
```

Determine best GAM from 7 formulations (see function)
```{r}
pklarvae_gams_t <- gam_selection(pk_larvae)
AIC(pklarvae_gams_t[[1]][[1]]) #29891
AIC(pklarvae_gams_t[[1]][[2]]) #29625
AIC(pklarvae_gams_t[[1]][[3]]) #29517
summary(pklarvae_gams_t[[2]]) # best model is spatial, retain all smooth terms
AIC(pklarvae_gams_t[[1]][[1]]) - AIC(pklarvae_gams_t[[2]]) # 374
summary(pkegg_gams_t[[1]][[1]])
summary(pkegg_gams_t[[1]][[2]])
```
...
var_ratio_pklarvae_month_t #0.217
...
var_ratio_pklarvae_space_t #0.244
```
...
ratio_fhsegg_geography_t #0.018
```
...
AIC(fhsegg_gams_t[[1]][[1]]) # 14738
AIC(fhsegg_gams_t[[1]][[2]]) # 14687
AIC(fhsegg_gams_t[[1]][[3]]) # 14612
summary(fhsegg_gams_t[[2]]) # best model is spatial VC
AIC(fhsegg_gams_t[[1]][[1]]) - AIC(fhsegg_gams_t[[2]]) # 126
```
...
ratio_fhslarvae_phenology_t # 0.012
...
ratio_fhslarvae_geography_t # 0.005
```
...
AIC(fhslarvae_gams_t[[1]][[1]]) #13418
AIC(fhslarvae_gams_t[[1]][[2]]) #13086
AIC(fhslarvae_gams_t[[1]][[3]]) #12967
summary(fhslarvae_gams_t[[2]]) # best model is spatial VC, keep all smooth terms
AIC(fhslarvae_gams_t[[1]][[1]]) - AIC(fhslarvae_gams_t[[2]]) # 452
```
...
var_ratio_fhslarvae_month_t #0.336
...
var_ratio_fhslarvae_space_t #0.358
```
...
AIC(yfsegg_gams_t[[1]][[1]]) # 531
AIC(yfsegg_gams_t[[1]][[2]]) # 512
AIC(yfsegg_gams_t[[1]][[3]]) # 515
summary(yfsegg_gams_t[[2]]) # best model is phenology VC, doy not significant though?
AIC(yfsegg_gams_t[[1]][[2]]) - AIC(yfsegg_gams_t[[2]]) # -3.4
```
...
```{r}
distribution_change(yfs_egg, yfsegg_vcgrids_t)
```
...
ratio_yfslarvae_phenology_t # 0.010
...
ratio_yfslarvae_geography_t # 0.017
```
...
AIC(yfslarvae_gams_t[[1]][[1]]) # 10345
AIC(yfslarvae_gams_t[[1]][[2]]) # 10219
AIC(yfslarvae_gams_t[[1]][[3]]) # 10025
summary(yfslarvae_gams_t[[2]]) # best is spatial VC, keep all smooth terms
AIC(yfslarvae_gams_t[[1]][[1]]) - AIC(yfslarvae_gams_t[[2]]) # 320
```
...
var_ratio_yfslarvae_month_t #0.115
...
var_ratio_yfslarvae_space_t #0.205
```
...
AIC(akpegg_gams_t[[1]][[1]]) # 14478
AIC(akpegg_gams_t[[1]][[2]]) # 14382
AIC(akpegg_gams_t[[1]][[3]]) # 14180
summary(akpegg_gams_t[[2]]) # best model is spatial VC, keep all terms
AIC(akpegg_gams_t[[1]][[1]]) - AIC(akpegg_gams_t[[2]]) # 298
```
...
ratio_akplarvae_phenology_t # 0.002
...
ratio_akplarvae_geography_t # 0.021
```
...
AIC(akplarvae_gams_t[[1]][[1]]) # 7067
AIC(akplarvae_gams_t[[1]][[2]]) # 6732
AIC(akplarvae_gams_t[[1]][[3]]) # 6644
summary(akplarvae_gams_t[[2]]) # best is spatial VC
AIC(akplarvae_gams_t[[1]][[1]]) - AIC(akplarvae_gams_t[[2]]) # 423
```
...
var_ratio_akplarvae_month_t # 0.299
...
var_ratio_akplarvae_space_t # 0.333
```
...
ratio_pcodlarvae_phenology_t # 0.009
...
ratio_pcodlarvae_geography_t # 0.001
```
...
AIC(pcodlarvae_gams_t[[1]][[1]]) # 8857
AIC(pcodlarvae_gams_t[[1]][[2]]) # 8673
AIC(pcodlarvae_gams_t[[1]][[3]]) # 8694
summary(pcodlarvae_gams_t[[2]]) # best is temporal VC
AIC(pcodlarvae_gams_t[[1]][[1]]) - AIC(pcodlarvae_gams_t[[2]]) # 185
```
...
var_ratio_pcodlarvae_month_t # 0.125
...
var_ratio_pcodlarvae_space_t #  0.123
```
...
ratio_nrslarvae_phenology_t # -0.0002
...
ratio_nrslarvae_geography_t # 0.005
```
...
AIC(nrslarvae_gams_t[[1]][[1]]) # 15563
AIC(nrslarvae_gams_t[[1]][[2]]) # 15450
AIC(nrslarvae_gams_t[[1]][[3]]) # 15415
summary(nrslarvae_gams_t[[2]]) # best is spatial VC
AIC(nrslarvae_gams_t[[1]][[1]]) - AIC(nrslarvae_gams_t[[2]]) # 148
```
...
var_ratio_nrslarvae_month_t # 0.116
...
var_ratio_nrslarvae_space_t # 0.129
```

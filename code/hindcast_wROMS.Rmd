...
source(here('code/functions', 'scale_temp.R'))
```
...
AIC(pk_egg_base_t) - AIC(pkegg_gams_t[[2]]) 
```
...
```{r}
pk_egg_temps <- scale_temp(pkegg_gams_t, pk_egg)
pk_egg_hindcast <- hindcast_cells(c(unique(pk_egg$year)), pk_egg, 134, 5, pkegg_gams_t, roms_temps, pk_egg_temps)
saveRDS(pk_egg_temps, here('data', 'pk_egg_temps')) # to use in predictions
...
Note variable coefficient by year for each
```{r}
pk_larvae_phenology_t <- gam(larvalcatchper10m2 ~ s(year, bs = 're') +
                               s(doy, k = 9, bs = "tp", m = 1) +
                               te(lon, lat, bs = "tp", m = 1) +
                               s(doy, by = mean_temp, bs = "tp", m = 1),
                             data = pk_larvae,
...
pk_larvae_geography_t <- gam(larvalcatchper10m2 ~ s(year, bs = 're') +
                               te(lon, lat, bs = "tp", m = 1) +
                               s(lon, lat, by = mean_temp, bs = "tp", m = 1) +
                               s(doy, k = 9, bs = "tp", m = 1),
                             data = pk_larvae,
...
AIC(pk_larvae_base_t) - AIC(pklarvae_gams_t[[2]])
```
...
Create prediction grid to compare with projections
```{r}
pk_larvae_temps <- scale_temp(pklarvae_gams_t, pk_larvae)
pk_larvae_hindcast <- hindcast_cells(c(unique(pk_larvae$year)), 
                                     pk_larvae, 134, 5, pklarvae_gams_t, roms_temps, pk_larvae_temps)
saveRDS(pk_larvae_temps, here('data', 'pk_larvae_temps')) # to use in predictions
...
## Flathead Sole
```{r}
fhs_egg_base_t <- formula_base(fhs_egg)
summary(fhs_egg_base_t)
AIC(fhs_egg_base_t) 

fhs_larvae_base_t <- formula_base(fhs_larvae)
summary(fhs_larvae_base_t)
AIC(fhs_larvae_base_t) 
```
...
```{r}
fhs_egg_phenology_t <- formula_base_pheno(fhs_egg)
summary(fhs_egg_phenology_t)
AIC(fhs_egg_phenology_t)

fhs_egg_geography_t <- formula_base_geog(fhs_egg)
...
AIC(fhsegg_gams_t[[1]][[1]]) 
AIC(fhsegg_gams_t[[1]][[2]]) 
AIC(fhsegg_gams_t[[1]][[3]]) 
summary(fhsegg_gams_t[[2]]) # best model is spatial VC
AIC(fhs_egg_base_t) - AIC(fhsegg_gams_t[[2]]) 
summary(fhsegg_gams_t[[1]][[1]])
summary(fhsegg_gams_t[[1]][[2]])
```
...
Create prediction grid to compare with projections
```{r}
fhs_egg_temps <- scale_temp(fhsegg_gams_t, fhs_egg)
fhs_egg_hindcast <- hindcast_cells(c(unique(fhs_egg$year)), 
                                     fhs_egg, 154, 6, fhsegg_gams_t, roms_temps, fhs_egg_temps)
saveRDS(fhs_egg_temps, here('data', 'fhs_egg_temps')) # to use in predictions
saveRDS(fhs_egg_hindcast, here('data', 'fhs_egg_hindcast'))
```
...
par(mfrow = c(1, 3), 
    mai = c(0.7, 0.6, 0.4, 0.4),
    family = "serif")
map_vc_presentation(fhs_egg, fhsegg_gams_t[[2]], fhsegg_vcgrids_t)
legend(125,
       0,
       legend = c('Mean SST', '+ 1C'),
       col = c('#000000', '#E69F00'),
...
Note variable coefficient by year for each
```{r}
fhs_larvae_phenology_t <- formula_base_pheno(fhs_larvae)
summary(fhs_larvae_phenology_t)
AIC(fhs_larvae_phenology_t)

fhs_larvae_geography_t <- formula_base_geog(fhs_larvae)
...
AIC(fhslarvae_gams_t[[1]][[1]]) 
AIC(fhslarvae_gams_t[[1]][[2]]) 
AIC(fhslarvae_gams_t[[1]][[3]]) 
summary(fhslarvae_gams_t[[2]]) # best model is spatial VC, keep all smooth terms
AIC(fhs_larvae_base_t) - AIC(fhslarvae_gams_t[[2]]) 
summary(fhslarvae_gams_t[[1]][[1]])
summary(fhslarvae_gams_t[[1]][[2]])
```
...
Create prediction grid to compare with projections
```{r}
fhs_larvae_temps <- scale_temp(fhslarvae_gams_t, fhs_larvae)
fhs_larvae_hindcast <- hindcast_cells(c(unique(fhs_larvae$year)), 
                                     fhs_larvae, 138, 5, fhslarvae_gams_t, roms_temps, fhs_larvae_temps)
saveRDS(fhs_larvae_temps, here('data', 'fhs_larvae_temps')) # to use in predictions
saveRDS(fhs_larvae_hindcast, here('data', 'fhs_larvae_hindcast'))
```
...
par(mfrow = c(1, 3), 
    mai = c(0.7, 0.6, 0.4, 0.4),
    family = "serif")
map_vc_presentation(fhs_larvae, fhslarvae_gams_t[[2]], fhslarvae_vcgrids_t)
legend(140,
       0.5,
       legend = c('Mean SST', '+ 1C'),
       col = c('#000000', '#E69F00'),
...
plot_var_coef2(fhslarvae_gams_t, fhs_larvae, pred_fhslarvae_t)
...
AIC(yfsegg_gams_t[[1]][[1]]) 
AIC(yfsegg_gams_t[[1]][[2]]) 
AIC(yfsegg_gams_t[[1]][[3]]) 
summary(yfsegg_gams_t[[2]]) # best model is phenology VC, doy not significant though?
AIC(yfsegg_gams_t[[1]][[2]]) - AIC(yfsegg_gams_t[[2]]) 
```
...
AIC(yfslarvae_gams_t[[1]][[1]])
AIC(yfslarvae_gams_t[[1]][[2]])
AIC(yfslarvae_gams_t[[1]][[3]]) 
summary(yfslarvae_gams_t[[2]]) # best is spatial VC, keep all smooth terms
AIC(yfs_larvae_base_t) - AIC(yfslarvae_gams_t[[2]]) 
...
Create prediction grid to compare with projections
```{r}
yfs_larvae_temps <- scale_temp1(yfslarvae_gams_t, yfs_larvae)
yfs_larvae_hindcast <- hindcast_cells(c(unique(yfs_larvae$year)), 
                                     yfs_larvae, 254, 9, yfslarvae_gams_t, roms_temps, yfs_larvae_temps)
saveRDS(yfs_larvae_temps, here('data', 'yfs_larvae_temps')) # to use in predictions
...
plot_var_coef2(yfslarvae_gams_t, yfs_larvae, pred_yfslarvae_t)
...
AIC(akpegg_gams_t[[1]][[1]]) 
AIC(akpegg_gams_t[[1]][[2]]) 
AIC(akpegg_gams_t[[1]][[3]]) 
summary(akpegg_gams_t[[2]]) # best model is spatial VC, keep all terms
AIC(akp_egg_base_t) - AIC(akpegg_gams_t[[2]]) 
...
Create prediction grid to compare with projections
```{r}
akp_egg_temps <- scale_temp(akpegg_gams_t, akp_egg)
akp_egg_hindcast <- hindcast_cells(c(unique(akp_egg$year)), 
                                     akp_egg, 134, 5, akpegg_gams_t, roms_temps, akp_egg_temps)
saveRDS(akp_egg_temps, here('data', 'akp_egg_temps')) # to use in predictions
...
AIC(akplarvae_gams_t[[1]][[1]]) 
AIC(akplarvae_gams_t[[1]][[2]]) 
AIC(akplarvae_gams_t[[1]][[3]]) 
summary(akplarvae_gams_t[[2]]) # best is spatial VC
AIC(akp_larvae_base_t) - AIC(akplarvae_gams_t[[2]])
...
Create prediction grid to compare with projections
```{r}
akp_larvae_temps <- scale_temp(akplarvae_gams_t, akp_larvae)
akp_larvae_hindcast <- hindcast_cells(c(unique(akp_larvae$year)), 
                                     akp_larvae, 140, 5, akplarvae_gams_t, roms_temps, akp_larvae_temps)
saveRDS(akp_larvae_temps, here('data', 'akp_larvae_temps')) # to use in predictions
...
plot_var_coef(akplarvae_gams_t, akp_larvae, pred_akplarvae_t)
...
AIC(pcodlarvae_gams_t[[1]][[1]]) 
AIC(pcodlarvae_gams_t[[1]][[2]]) 
AIC(pcodlarvae_gams_t[[1]][[3]]) 
summary(pcodlarvae_gams_t[[2]]) # best is temporal VC
AIC(pcod_larvae_base_t) - AIC(pcodlarvae_gams_t[[2]]) 
...
Create prediction grid to compare with projections
```{r}
pcod_larvae_temps <- scale_temp1(pcodlarvae_gams_t, pcod_larvae)
pcod_larvae_hindcast <- hindcast_cells(c(unique(pcod_larvae$year)), 
                                       pcod_larvae, 134, 5, pcodlarvae_gams_t, roms_temps, pcod_larvae_temps)
saveRDS(pcod_larvae_temps, here('data', 'pcod_larvae_temps')) # to use in predictions
...
AIC(nrslarvae_gams_t[[1]][[1]]) 
AIC(nrslarvae_gams_t[[1]][[2]]) 
AIC(nrslarvae_gams_t[[1]][[3]]) 
summary(nrslarvae_gams_t[[2]]) # best is spatial VC
AIC(nrs_larvae_base_t) - AIC(nrslarvae_gams_t[[2]]) 
...
Create prediction grid to compare with projections
```{r}
nrs_larvae_temps <- scale_temp1(nrslarvae_gams_t, nrs_larvae)
nrs_larvae_hindcast <- hindcast_cells(c(unique(nrs_larvae$year)), 
                                     nrs_larvae, 134, 5, nrslarvae_gams_t, roms_temps, nrs_larvae_temps)
saveRDS(nrs_larvae_temps, here('data', 'nrs_larvae_temps')) # to use in predictions
...
par(mfrow = c(9, 5),
    ...
    mgp = c(10, 4, 0),
    family = "serif")
plot_variable_multipanel(pkegg_gams_t,
              covariate = 2,
              bounds = c(-6.5, 9),
              "Day of Year",
              "Species Abundance Anomalies",
...
location_plot(pkegg_gams_t,
              pk_egg)
plot_variable_multipanel(pklarvae_gams_t,
              covariate = 2,
              bounds = c(-6.5, 9),
              "Day of Year",
              "Species Abundance Anomalies",
...
location_plot(pklarvae_gams_t,
              pk_larvae)
plot_variable_multipanel(fhsegg_gams_t,
              covariate = 2,
              bounds = c(-6.5, 3),
              "Day of Year",
              "Species Abundance Anomalies",
...
location_plot(fhsegg_gams_t,
              fhs_egg)
plot_variable_multipanel(fhslarvae_gams_t,
              covariate = 2,
              bounds = c(-6.5, 3),
              "Day of Year",
              "Species Abundance Anomalies",
...
location_plot(fhslarvae_gams_t,
              fhs_larvae)
plot_variable_multipanel(akpegg_gams_t,
              covariate = 2,
              bounds = c(-4, 4),
              "Day of Year",
              "Species Abundance Anomalies",
...
location_plot(akpegg_gams_t,
              akp_egg)
plot_variable_multipanel(akplarvae_gams_t,
              covariate = 2,
              bounds = c(-4, 4),
              "Day of Year",
              "Species Abundance Anomalies",
...
location_plot(akplarvae_gams_t,
              akp_larvae)
plot_variable_multipanel(yfslarvae_gams_t,
              covariate = 2,
              bounds = c(-3, 3),
              "Day of Year",
              "Species Abundance Anomalies",
...
location_plot(yfslarvae_gams_t,
              yfs_larvae)
plot_variable_multipanel(pcodlarvae_gams_t,
              covariate = 2,
              bounds = c(-4, 3.5),
              "Day of Year",
              "Species Abundance Anomalies",
...
location_plot(pcodlarvae_gams_t,
              pcod_larvae)
plot_variable_multipanel(nrslarvae_gams_t,
              covariate = 2,
              bounds = c(-4, 2),
              "Day of Year",
              "Species Abundance Anomalies",
...
plot_variable_multipanel(pkegg_gams_t,
              ...
              "Species Abundance Anomalies",
              "s",
              bounds_min = min(pk_egg$doy, na.rm = TRUE),
              bounds_max = max(pk_egg$doy, na.rm = TRUE))
plot_variable_multipanel(pkegg_gams_t,
              covariate = 4,
              bounds = c(-6, 8),
              expression(paste("Temperature ("^0, 'C)')),
              " ",
              "n",
              bounds_min = min(pk_larvae$roms_temperature, na.rm = TRUE),
              bounds_max = max(pk_egg$roms_temperature, na.rm = TRUE))
plot_variable_multipanel(pkegg_gams_t,
              covariate = 4,
              bounds = c(-6, 8),
              "Salinity",
              " ",
              "n",
              bounds_min = min(pk_egg$roms_salinity, na.rm = TRUE),
              bounds_max = max(pk_egg$roms_salinity, na.rm = TRUE))
phenology_change(pkegg_vcgrids_t, 
                 min(pk_egg$doy, na.rm = TRUE),
                 max(pk_egg$doy, na.rm = TRUE))
...
plot_variable_multipanel(pklarvae_gams_t,
              ...
              " ",
              "n",
              bounds_min = min(pk_larvae$roms_salinity, na.rm = TRUE),
              bounds_max = max(pk_larvae$roms_salinity, na.rm = TRUE))
...
plot_variable_multipanel(fhsegg_gams_t,
              ...
              "Species Abundance Anomalies",
              "s",
              bounds_min = min(fhs_egg$doy, na.rm = TRUE),
              bounds_max = max(fhs_egg$doy, na.rm = TRUE))
plot_variable_multipanel(fhsegg_gams_t,
              covariate = 4,
              bounds = c(-6.5, 3),
              expression(paste("Temperature ("^0, 'C)')),
              " ",
              "n",
              bounds_min = min(fhs_egg$roms_temperature, na.rm = TRUE),
              bounds_max = max(fhs_egg$roms_temperature, na.rm = TRUE))
plot_variable_multipanel(fhsegg_gams_t,
              covariate = 5,
              bounds = c(-6.5, 3),
              "Salinity",
              " ",
              "n",
              bounds_min = min(fhs_egg$roms_salinity, na.rm = TRUE),
              bounds_max = max(fhs_egg$roms_salinity, na.rm = TRUE))
phenology_change(fhsegg_vcgrids_t, 
                 min(fhs_egg$doy, na.rm = TRUE),
                 max(fhs_egg$doy, na.rm = TRUE))
...
plot_variable_multipanel(fhslarvae_gams_t,
              ...
              "Species Abundance Anomalies",
              "s",
              bounds_min = min(fhs_larvae$doy, na.rm = TRUE),
              bounds_max = max(fhs_larvae$doy, na.rm = TRUE))
plot_variable_multipanel(fhslarvae_gams_t,
              covariate = 4,
              bounds = c(-6.5, 3),
              expression(paste("Temperature ("^0, 'C)')),
              " ",
              "n",
              bounds_min = min(fhs_larvae$roms_temperature, na.rm = TRUE),
              bounds_max = max(fhs_larvae$roms_temperature, na.rm = TRUE))
plot_variable_multipanel(fhslarvae_gams_t,
              covariate = 5,
              bounds = c(-6.5, 3),
              "Salinity",
              " ",
              "n",
              bounds_min = min(fhs_larvae$roms_salinity, na.rm = TRUE),
              bounds_max = max(fhs_larvae$roms_salinity, na.rm = TRUE))
...
plot_variable_multipanel(akpegg_gams_t,
              ...
              "Species Abundance Anomalies",
              "s",
              bounds_min = min(akp_egg$doy, na.rm = TRUE),
              bounds_max = max(akp_larvae$doy, na.rm = TRUE))
plot_variable_multipanel(akpegg_gams_t,
              covariate = 4,
              bounds = c(-4, 4),
              expression(paste("Temperature ("^0, 'C)')),
              " ",
              "n",
              bounds_min = min(akp_egg$roms_temperature, na.rm = TRUE),
              bounds_max = max(akp_larvae$roms_temperature, na.rm = TRUE))
plot_variable_multipanel(akpegg_gams_t,
              covariate = 5,
              bounds = c(-4, 4),
              "Salinity",
              " ",
              "n",
              bounds_min = min(akp_egg$roms_salinity, na.rm = TRUE),
              bounds_max = max(akp_egg$roms_salinity, na.rm = TRUE))
phenology_change(akpegg_vcgrids_t, 
                 min(akp_egg$doy, na.rm = TRUE),
                 max(akp_larvae$doy, na.rm = TRUE))
...
plot_variable_multipanel(yfslarvae_gams_t,
              ...
              "Species Abundance Anomalies",
              "s",
              bounds_min = min(yfs_larvae$doy, na.rm = TRUE),
              bounds_max = max(yfs_larvae$doy, na.rm = TRUE))
plot_variable_multipanel(yfslarvae_gams_t,
              covariate = 4,
              bounds = c(-3, 2),
              expression(paste("Temperature ("^0, 'C)')),
              " ",
              "n",
              bounds_min = min(yfs_larvae$roms_temperature, na.rm = TRUE),
              bounds_max = max(yfs_larvae$roms_temperature, na.rm = TRUE))
plot_variable_multipanel(yfslarvae_gams_t,
              covariate = 5,
              bounds = c(-3, 2),
              "Salinity",
              " ",
              "n",
              bounds_min = min(yfs_larvae$roms_salinity, na.rm = TRUE),
              bounds_max = max(yfs_larvae$roms_salinity, na.rm = TRUE))
phenology_change(yfslarvae_vcgrids_t, 
                 min(yfs_larvae$doy, na.rm = TRUE),
                 max(yfs_larvae$doy, na.rm = TRUE))
...
plot_variable_multipanel(pcodlarvae_gams_t,
              ...
              "Species Abundance Anomalies",
              "s",
              bounds_min = min(pcod_larvae$doy, na.rm = TRUE),
              bounds_max = max(pcod_larvae$doy, na.rm = TRUE))
plot_variable_multipanel(pcodlarvae_gams_t,
              covariate = 4,
              bounds = c(-5.5, 2.5),
              expression(paste("Temperature ("^0, 'C)')),
              " ",
              "n",
              bounds_min = min(pcod_larvae$roms_temperature, na.rm = TRUE),
              bounds_max = max(pcod_larvae$roms_temperature, na.rm = TRUE))
plot_variable_multipanel(pcodlarvae_gams_t,
              covariate = 5,
              bounds = c(-5.5, 2.5),
              "Salinity",
              " ",
              "n",
              bounds_min = min(pcod_larvae$roms_salinity, na.rm = TRUE),
              bounds_max = max(pcod_larvae$roms_salinity, na.rm = TRUE))
phenology_change(pcodlarvae_vcgrids_t, 
                 min(pcod_larvae$doy, na.rm = TRUE),
                 max(pcod_larvae$doy, na.rm = TRUE))
...
plot_variable_multipanel(nrslarvae_gams_t,
              ...
              "Species Abundance Anomalies",
              "s",
              bounds_min = min(nrs_larvae$doy, na.rm = TRUE),
              bounds_max = max(nrs_larvae$doy, na.rm = TRUE))
plot_variable_multipanel(nrslarvae_gams_t,
              covariate = 4,
              bounds = c(-5, 2),
              expression(paste("Temperature ("^0, 'C)')),
              " ",
              "n",
              bounds_min = min(nrs_larvae$roms_temperature, na.rm = TRUE),
              bounds_max = max(nrs_larvae$roms_temperature, na.rm = TRUE))
plot_variable_multipanel(nrslarvae_gams_t,
              covariate = 5,
              bounds = c(-5, 2),
              "Salinity",
              " ",
              "n",
              bounds_min = min(nrs_larvae$roms_salinity, na.rm = TRUE),
              bounds_max = max(nrs_larvae$roms_salinity, na.rm = TRUE))
phenology_change(nrslarvae_vcgrids_t, 
                 min(nrs_larvae$doy, na.rm = TRUE),
                 max(nrs_larvae$doy, na.rm = TRUE))

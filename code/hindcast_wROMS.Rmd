...
Cross validation
```{r}
fhslarvae_cc <- gam_cv(fhs_larvae)
fhslarvae_cc
```
...
Cross validation
```{r}
yfslarvae_cc <- gam_cv(yfs_larvae)
yfslarvae_cc
```
...
Cross validation
```{r}
akplarvae_cc <- gam_cv(akp_larvae)
akplarvae_cc
```
...
Cross validation
```{r}
pcodlarvae_cc <- gam_cv(pcod_larvae)
pcodlarvae_cc
```
...
Cross validation
```{r}
nrslarvae_cc <- gam_cv(nrs_larvae)
nrslarvae_cc
```
...
```{r}
# Create figures for pollock larvae DOY & temp, AKP DOY
# remotes::install_github("gavinsimpson/gratia")
library(gratia)
smooth_pklarvae_doy <- smooth_estimates(pklarvae_gams_t[[2]],
                                        smooth = "s(doy)", 
                                        data = pk_larvae) %>%
  add_confint()
smooth_pklarvae_temp <- smooth_estimates(pklarvae_gams_t[[2]],
                                         smooth = "s(roms_temperature)", 
                                         data = pk_larvae) %>%
  add_confint()
smooth_akpegg_doy <- smooth_estimates(akpegg_gams_t[[2]],
                                      smooth = "s(doy)",
                                      data = akp_egg) %>%
  add_confint()
smooth_akplarvae_doy <- smooth_estimates(akplarvae_gams_t[[2]],
                                         smooth = "s(doy)",
                                         data = akp_larvae) %>%
  add_confint()
smooth_akpegg_temp <- smooth_estimates(akpegg_gams_t[[2]],
                                       smooth = "s(roms_temperature)",
                                       data = akp_egg) %>%
  add_confint()
smooth_akplarvae_temp <- smooth_estimates(akplarvae_gams_t[[2]],
                                          smooth = "s(roms_temperature)",
                                          data = akp_larvae) %>%
  add_confint()

pklarvae_doy <- ggplot(smooth_pklarvae_doy,
                       aes(x = doy, y = est)) +
  ...
  theme_classic() +
  labs(x = "Day of Year",
       y = "Species Abundance Anomalies") +
  ggplot2::theme(axis.text = element_text(family = "serif", size = 22),
                 ...
                 strip.text = element_text(family = "serif", size = 22),
                 panel.border = element_rect(color = "black", fill = NA, linewidth = 0.4),
                 plot.margin = margin(20, 30, 5, 30),
                 legend.position = "none")

pklarvae_temp <- ggplot(smooth_pklarvae_temp,
                        aes(x = roms_temperature, y = est)) +
  geom_ribbon(aes(ymin = lower_ci,
                  ymax = upper_ci),
              alpha = 1,
              fill = "lightsalmon2") +
  geom_line() +
  xlim(c(min(pk_larvae$roms_temperature),
         max(pk_egg$roms_temperature))) +
  geom_rug(sides = "b", 
           data = pk_larvae, 
           aes(x = pk_larvae$roms_temperature),
           inherit.aes = F) +
  ylim(c(-6, 8)) +
  theme_classic() +
  labs(x = "Temperature \u00B0C",
       y = "") +
  ggplot2::theme(axis.text = element_text(family = "serif", size = 26),
                 axis.text.x = element_text(vjust = .25),
                 axis.text.y = element_blank(),
                 axis.title = element_text(family = "serif", size = 26),
                 axis.title.x = element_text(vjust = 0.75),
                 axis.title.y = element_blank(),
                 strip.text = element_text(family = "serif", size = 26),
                 axis.ticks.y = element_blank(),
                 plot.margin = margin(5, 5, 5, 30),
                 panel.border = element_rect(color = "black", fill = NA, linewidth = 0.4),
                 legend.position = "none")

akpegg_doy <- ggplot(smooth_akpegg_doy,
                     aes(x = doy, y = est)) +
  geom_ribbon(aes(ymin = lower_ci,
                  ymax = upper_ci),
              alpha = 1,
...
akplarvae_doy <- ggplot(smooth_akplarvae_doy,
                     aes(x = doy, y = est)) +
  geom_ribbon(aes(ymin = lower_ci,
                  ymax = upper_ci),
              alpha = 1,
...
akpegg_temp <- ggplot(smooth_akpegg_temp,
                     aes(x = roms_temperature, y = est)) +
  geom_ribbon(aes(ymin = lower_ci,
                  ymax = upper_ci),
              alpha = 1,
              fill = "lightsalmon2") +
  geom_line() +
  xlim(c(min(akp_egg$roms_temperature),
         max(akp_larvae$roms_temperature))) +
  geom_rug(sides = "b", 
           data = akp_egg, 
           aes(x = akp_egg$roms_temperature),
           inherit.aes = F) +
  ylim(c(-4, 4.2)) +
  theme_classic() +
  labs(x = "Temperature \u00B0C",
...
akplarvae_temp <- ggplot(smooth_akplarvae_temp,
                     aes(x = roms_temperature, y = est)) +
  geom_ribbon(aes(ymin = lower_ci,
                  ymax = upper_ci),
              alpha = 1,
  ...
  geom_rug(sides = "b", 
           data = akp_larvae, 
           aes(x = akp_larvae$roms_temperature),
           inherit.aes = F) +
  ylim(c(-4, 4.2)) +
  theme_classic() +
  labs(x = "Temperature \u00B0C",
...
tiff(here('results',
     ...
     res = 300)
par(mfrow = c(9, 5),
    mar = c(15, 15, 5, 1.5) + 0.1,
    oma = c(4, 35, 10, 1),
    mgp = c(10, 4, 0),
    family = "serif")
plot_variable_pk(pkegg_gams_t,
                 covariate = 2,
                 bounds = c(-6, 8),
                 "Day of Year",
                 "Species Abundance Anomalies",

...
AIC(pkegg_gams_t[[1]][[1]]) - AIC(pkegg_gams_t[[1]][[3]]) # 1.63                    
```
...
ratio_pklarvae_phenology_t # 0.080
...
ratio_pklarvae_geography_t # 0.167
```

Determine best GAM from 7 formulations (see function)
```{r}
pklarvae_gams_t <- gam_selection(pk_larvae)
AIC(pklarvae_gams_t[[1]][[1]]) #29463
AIC(pklarvae_gams_t[[1]][[2]]) #29208
AIC(pklarvae_gams_t[[1]][[3]]) #29356
summary(pklarvae_gams_t[[2]]) # best model is temporal, retain all smooth terms
AIC(pklarvae_gams_t[[1]][[1]]) - AIC(pklarvae_gams_t[[2]]) #255
```
...
var_ratio_pklarvae_month_t #0.205
...
var_ratio_pklarvae_space_t #0.173
```
...
AIC(fhsegg_gams_t[[1]][[1]]) # 13021
AIC(fhsegg_gams_t[[1]][[2]]) # 12668
AIC(fhsegg_gams_t[[1]][[3]]) # 12887
summary(fhsegg_gams_t[[2]]) # best model is doy VC
AIC(fhsegg_gams_t[[1]][[1]]) - AIC(fhsegg_gams_t[[2]]) #353
```
...
ratio_fhslarvae_phenology_t # 0.017
...
ratio_fhslarvae_geography_t # 0.030
```
...
AIC(fhslarvae_gams_t[[1]][[1]]) #14099
AIC(fhslarvae_gams_t[[1]][[2]]) #14050
AIC(fhslarvae_gams_t[[1]][[3]]) #14080
summary(fhslarvae_gams_t[[2]]) # best model is phenology VC, keep all smooth terms
AIC(fhslarvae_gams_t[[1]][[1]]) - AIC(fhslarvae_gams_t[[2]]) #49
```
...
var_ratio_fhslarvae_month_t #0.078
...
var_ratio_fhslarvae_space_t #0.066
```
...
ratio_yfslarvae_phenology_t # -0.0006
...
ratio_yfslarvae_geography_t # -0.0017
```
...
AIC(yfslarvae_gams_t[[1]][[1]]) # 7512
AIC(yfslarvae_gams_t[[1]][[2]]) # 7367
AIC(yfslarvae_gams_t[[1]][[3]]) # 7317
summary(yfslarvae_gams_t[[2]]) # best is spatial VC, keep all smooth terms
AIC(yfslarvae_gams_t[[1]][[1]]) - AIC(yfslarvae_gams_t[[2]]) # 195
```
...
var_ratio_yfslarvae_month_t #0.147
...
var_ratio_yfslarvae_space_t #0.177
```
...
AIC(akpegg_gams_t[[1]][[1]]) # 14184
AIC(akpegg_gams_t[[1]][[2]]) # 14125
AIC(akpegg_gams_t[[1]][[3]]) # 14114
summary(akpegg_gams_t[[2]]) # best model is location VC, keep all terms
AIC(akpegg_gams_t[[1]][[1]]) - AIC(akpegg_gams_t[[2]]) #70
```
...
var_ratio_akpegg_space_t # 0.305
```
...
ratio_akplarvae_phenology_t # -0.0007
...
ratio_akplarvae_geography_t # 0.030
```
...
AIC(akplarvae_gams_t[[1]][[1]]) # 5634
AIC(akplarvae_gams_t[[1]][[2]]) # 5447
AIC(akplarvae_gams_t[[1]][[3]]) # 5485
summary(akplarvae_gams_t[[2]]) # best is phenology VC
AIC(akplarvae_gams_t[[1]][[1]]) - AIC(akplarvae_gams_t[[2]]) #188
```
...
var_ratio_akplarvae_month_t # 0.254
...
var_ratio_akplarvae_space_t # 0.236
```
...
ratio_pcodlarvae_phenology_t # 0.021
...
AIC(pcodlarvae_gams_t[[1]][[1]]) # 8229
AIC(pcodlarvae_gams_t[[1]][[2]]) # 8087
AIC(pcodlarvae_gams_t[[1]][[3]]) # 8124
summary(pcodlarvae_gams_t[[2]]) # best is phenology VC
AIC(pcodlarvae_gams_t[[1]][[1]]) - AIC(pcodlarvae_gams_t[[2]]) # 143
```
...
var_ratio_pcodlarvae_month_t # 0.146
...
var_ratio_pcodlarvae_space_t #  0.131
```
...
ratio_nrslarvae_phenology_t # 0.051
...
ratio_nrslarvae_geography_t # 0.053
```
...
AIC(nrslarvae_gams_t[[1]][[1]]) # 14559
AIC(nrslarvae_gams_t[[1]][[2]]) # 14521
AIC(nrslarvae_gams_t[[1]][[3]]) # 14505
summary(nrslarvae_gams_t[[2]]) # best is location VC
AIC(nrslarvae_gams_t[[1]][[1]]) - AIC(nrslarvae_gams_t[[2]]) # 55
```
...
var_ratio_nrslarvae_month_t # 0.125
...
var_ratio_nrslarvae_space_t # 0.130
```

...
map_vc <- function(data, gam, grid, grid2, title){
  nlat = 80
  nlon = 120
  ...
  plot(grid2$doy,
       ...
       ylim = range(c(grid2$pred_up,
                      ...
                      grid2$pred_lw,
                      grid2$pred2_lw)),
       xlim = c(min(grid2$doy), max(grid2$doy)),
       col = '#000000',
       lwd = 2,
       xlab = 'Day of the year',
       ylab = 'Egg density ln(n/10m2)',
...
grid_pkegg_avg_t$pred <- predict(pk_egg_base_t, newdata = grid_pkegg_avg_t)
...
grid_pkegg_avg_t2 <-  data.frame('lon' = rep(-170, 100),
                                ...
                                'doy' = seq(min(pk_egg$doy), 
                                            ...
                                            length = 100),
                                'year' = rep(2014, 100))
grid_pkegg_avg_t2$pred <- predict(pk_egg_base_t, newdata = grid_pkegg_avg_t2)
grid_pkegg_avg_t2$se <- predict(pk_egg_base_t, newdata = grid_pkegg_avg_t2, se = T)[[2]]

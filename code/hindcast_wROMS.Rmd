...
Base model: Currently using only presence in a Gaussian GAM, may switch to Tweedie With Gaussian, multiplication by binomial GAM required (two-stage)
```{r}
...
grid_vc_larvaepk$pred <- predict(pk_larvae_geography, newdata = grid_vc_larvaepk)
grid_vc_larvaepk$se <- predict(pk_larvae_geography, newdata = grid_vc_larvaepk, se = T)[[2]]
